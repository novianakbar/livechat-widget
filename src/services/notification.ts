...
export class NotificationService {
  ...
  private playBeep(frequency?: number): void {
    ...
    try {
      ...
      gainNode.gain.exponentialRampToValueAtTime(this.RAMP_VALUE, currentTime + this.RAMP_DURATION);
   ...
   */
  async playBeepVariation(type: 'success' | 'warning' | 'error' | 'info' = 'info'): Promise<void> {
    if (!this.audioContext) return;
    ...
    try {
      ...
      const variations = {
        success: [{ freq: 800, delay: 0 }, { freq: 1000, delay: 150 }], // High-low success sound
        warning: [{ freq: 600, delay: 0 }], // Medium frequency warning
        error: [{ freq: 400, delay: 0 }, { freq: 400, delay: 200 }], // Low frequency double beep
   ...
   */
  async notifyNewMessage(sender: string, message: string, options: {
    playSound?: boolean;
    showBrowser?: boolean;
    vibrate?: boolean;
    icon?: string;
